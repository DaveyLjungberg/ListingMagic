...
import type { FeaturesRequest, FeaturesResponse, ErrorResponse } from "@/types/api";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call Python backend
    const backendResponse = await fetch(`${BACKEND_URL}/api/generate-features`, {
      method: "POST",
      headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: errorData.detail || "Failed to generate features",
    ...
    const data: FeaturesResponse = await backendResponse.json();
