/**
 * ListingGopher Review API Route
 *
 * Reviews uploaded documents for readiness, issues, and missing information.
 * Calls Python backend for AI analysis.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // For review, we typically need documents to review
    if (documentUrls.length === 0) {
      return NextResponse.json(
        { success: false, error: "no_documents", message: "Please upload documents to review first" },
        { status: 400 }
      );
    }
    ...
    console.log('Calling Python backend:', {
      endpoint: `${BACKEND_URL}/generate/review`,
    ...
    let backendResponse;
    try {
      backendResponse = await fetch(`${BACKEND_URL}/generate/review`, {
        method: "POST",
    ...
    if (!backendResponse.ok) {
      ...
      if (backendResponse.status === 504) {
        return NextResponse.json(
          { success: false, error: "generation_timeout", message: "Review took too long" },
          { status: 504 }
        );
      }

      return NextResponse.json(
        { success: false, error: "backend_error", message: errorData.detail || "Review failed" },
        { status: 502 }
      );
    }
