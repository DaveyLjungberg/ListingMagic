/**
 * ListingGopher Summarize API Route
 *
 * Summarizes key points from uploaded documents.
 * Calls Python backend for AI summarization.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (documentUrls.length === 0) {
      return NextResponse.json(
        { success: false, error: "no_documents", message: "Please upload documents to summarize first" },
        { status: 400 }
      );
    }
    ...
    if (!backendResponse.ok) {
      ...
      if (backendResponse.status === 504) {
        return NextResponse.json(
          { success: false, error: "generation_timeout", message: "Summarization took too long" },
          { status: 504 }
        );
      }

      return NextResponse.json(
        { success: false, error: "backend_error", message: errorData.detail || "Summarization failed" },
        { status: 502 }
      );
    }
    ...
    console.error("Summarize generation error:", error);
    return NextResponse.json(
      {
        success: false,
